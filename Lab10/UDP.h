...
public:
    ...
    static int UDP_FillSockAddr(struct sockaddr_in*addr,char*hostname, int port)
    {
    ...
    static int UDP_Read(int sd, struct sockaddr_in*addr,char*buffer, int n)
    {
        int len = sizeof(struct sockaddr_in);
        return recvfrom(sd, buffer, n, 0, (struct sockaddr*)addr, (socklen_t*) &len);
    }

    static int UDP_Write(int sd, struct sockaddr_in*addr,char*buffer, int n)
    {
